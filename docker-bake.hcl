...
#
# export IMAGE_NAME="$(basename -s .git "$(git remote get-url origin)")";
variable "IMAGE_NAME" {default="nomad-jobspecs"}
# ────────────────────────────────────────────────────────────
...
#
# By default, the final basename -s .git of the image is going to be `${var.IMAGE_NAME}:<target-name>`
#
...
#
# for instance we can include release version by setting `TAG` environment variable which would make the final image basename -s .git to be
...
# │                   image build targets                    │
# ╰──────────────────────────────────────────────────────────╯
# This target builds Nomad inside alpine container
#
#
# Make sure that you have already authenticated against DockerHub
# Docker image registry before running a build job.
#
# [ NOTE ] => Alpine linux image supports multiplatform builds but Nomad's arm64 build has not been tested yet.
# ─── SNIPPETS ───────────────────────────────────────────────────────────────────
# ❯ Build for both architectures without pushing to registry.
#
# LOCAL=true docker buildx bake --builder "$(basename -s .git "$(git remote get-url origin)")" "nomad"
#
# ❯ Build only for AMD64 architecture without pushing to registry
#
# LOCAL=true ARM64=false AMD64=true docker buildx bake --builder "$(basename -s .git "$(git remote get-url origin)")" "nomad"
#
# ❯ Build only for ARM64 architecture without pushing to registry
# ────────────────────────────────────────────────────────────
target "nomad" {
    context="."
    dockerfile = "contrib/docker/nomad/Dockerfile"
    tags = [
        equal("",TAG) ? "${MAIN_REGISTRY_HOSTNAME}/${MAIN_REGISTRY_USERNAME}/${IMAGE_NAME}:nomad": "${MAIN_REGISTRY_HOSTNAME}/${MAIN_REGISTRY_USERNAME}/${IMAGE_NAME}:nomad-${TAG}",
    ]
    platforms = [
        equal(AMD64,true) ?"linux/amd64":"",
        equal(ARM64,true) ?"linux/arm64":"",
    ]
    cache-from   = [equal("",TAG) ? "type=registry,mode=max,ref=${CACHE_REGISTRY_HOSTNAME}/${CACHE_REGISTRY_USERNAME}/${IMAGE_NAME}-cache/nomad": "type=registry,mode=max,ref=${CACHE_REGISTRY_HOSTNAME}/${CACHE_REGISTRY_USERNAME}/${IMAGE_NAME}-cache:nomad-${TAG}"]
    cache-to   = [equal(LOCAL,true) ? "" : equal("",TAG) ? "type=registry,mode=max,ref=${CACHE_REGISTRY_HOSTNAME}/${CACHE_REGISTRY_USERNAME}/${IMAGE_NAME}-cache/nomad": "type=registry,mode=max,ref=${CACHE_REGISTRY_HOSTNAME}/${CACHE_REGISTRY_USERNAME}/${IMAGE_NAME}-cache:nomad-${TAG}"]
